...
#define CHECK_ERR(FN)                                                \
    {                                                                \
        auto res = FN;                                               \
        if (FAILED(res)) {                                           \
            std::cout << #FN << " failed due to " << std::hex << res \
                      << std::endl                                   \
                      << std::flush;                                 \
...
int main(int argc, const char **argv) {
    ...
    SDL_Window *window = SDL_CreateWindow("SDL2 + DX12", SDL_WINDOWPOS_CENTERED,
    ...
    CHECK_ERR(D3D12CreateDevice(nullptr, D3D_FEATURE_LEVEL_12_1,
                                IID_PPV_ARGS(&device)));
    ...
    ComPtr<ID3D12Resource> render_targets[2];
    {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            device->CreateRenderTargetView(render_targets[i].Get(), nullptr,
                                           rtv_handle);
    ...
    // Setup the pipeline state and the shaders which will be used for
    // rasterizing the triangle

    // Make the command list
    ComPtr<ID3D12GraphicsCommandList4> cmd_list;
    CHECK_ERR(device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
    ...
    ComPtr<ID3D12Resource> readback_img_buf;
    {
        ...
        CHECK_ERR(device->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &res_desc,
            D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
            IID_PPV_ARGS(&readback_img_buf)));
    }
...

    {
        ...
        // Place the vertex data in an upload heap first, then do a GPU-side
        // copy into a default heap (resident in VRAM)
        CHECK_ERR(device->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &res_desc,
        ...
        std::memcpy(mapping, vertex_data.data(),
        ...
        res_barrier.Transition.Subresource =
            D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        res_barrier.Transition.StateBefore =
        ...
        rt_geom_desc.Triangles.VertexBuffer.StartAddress =
        ...
        // Determine bound of much memory the accel builder may need and
        // allocate it
        ...
        D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO prebuild_info = {
            0};
        device->GetRaytracingAccelerationStructurePrebuildInfo(&as_inputs,
                                                               &prebuild_info);
        ...
        std::cout << "Bottom level AS will use at most "
                  << prebuild_info.ResultDataMaxSizeInBytes
                  << " bytes, and scratch of "
                  << prebuild_info.ScratchDataSizeInBytes << " bytes\n";
        ...
        // Now build the bottom level acceleration structure
        {
            ...
            build_desc.DestAccelerationStructureData =
                bottom_level_as->GetGPUVirtualAddress();
            build_desc.ScratchAccelerationStructureData =
                bottom_scratch->GetGPUVirtualAddress();
            cmd_list->BuildRaytracingAccelerationStructure(&build_desc, 0,
                                                           NULL);

            // Insert a barrier to wait for the bottom level AS to complete
            // before the top level build is started
        ...
        // Now build the top-level accel. structure over our one instantiation
        ...
        as_inputs.DescsLayout = D3D12_ELEMENTS_LAYOUT_ARRAY;
        as_inputs.NumDescs = 1;
        // Note: we don't need to give it the instance descriptors to estimate
        // the size, just when we want to build the data. Just unset the geom
        // descs. now since we're re-using it
        ...
        std::cout << "Top level AS will use at most "
                  << prebuild_info.ResultDataMaxSizeInBytes
                  << " bytes, and scratch of "
                  << prebuild_info.ScratchDataSizeInBytes << " bytes\n";
        ...
        CHECK_ERR(device->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &res_desc,
            D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, nullptr,
            IID_PPV_ARGS(&top_level_as)));

        res_desc.Width = prebuild_info.ScratchDataSizeInBytes;
        CHECK_ERR(device->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &res_desc,
            D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
            IID_PPV_ARGS(&top_scratch)));
        ...
        // We want to write from the CPU to this buffer so place in the upload
        // heap
        props.Type = D3D12_HEAP_TYPE_UPLOAD;
        res_desc.Flags = D3D12_RESOURCE_FLAG_NONE;
        CHECK_ERR(device->CreateCommittedResource(
        ...
        // Now build the top level acceleration structure
        {
            ...
            build_desc.Inputs = as_inputs;
            build_desc.DestAccelerationStructureData =
                top_level_as->GetGPUVirtualAddress();
            build_desc.ScratchAccelerationStructureData =
                top_scratch->GetGPUVirtualAddress();
            cmd_list->BuildRaytracingAccelerationStructure(&build_desc, 0,
                                                           NULL);
            ...
            // We won't need to run anything else on the command list until
            // render time, so submit this work now
            CHECK_ERR(cmd_list->Close());
        ...
        const std::vector<std::wstring> export_fcn_names = {L"RayGen", L"Miss",
                                                            L"ClosestHit"};
        ...
        // Make the shader config which defines the maximum size in bytes for
        // the ray payload and attribute structure
        ...
        // don't make use of a local root signature (no reads from
        // buffers/textures)
        {
            ...
            // TODO WILL: Even though this is just one descriptor each, I think
            // this must always be passed as a descriptor table to allow for per
            // hit group params
            ...
            // SRV param for the output image buffer, which is the second entry
            // in the table(?)
            ...
            auto res = D3D12SerializeRootSignature(&root_desc,
            ...
            CHECK_ERR(device->CreateRootSignature(
                0, signature_blob->GetBufferPointer(),
                signature_blob->GetBufferSize(),
                IID_PPV_ARGS(&rt_root_signature)));
        }
        // Make dummy global root signature
        {
            ...
            CHECK_ERR(device->CreateRootSignature(
                0, signature_blob->GetBufferPointer(),
                signature_blob->GetBufferSize(),
                IID_PPV_ARGS(&rt_root_signature_dummy_global)));
        }

        // Now we can build the raytracing pipeline. It's made of a bunch of
        // subobjects that describe the shader code libraries, hit groups, root
        // signature associations and some other config stuff
        ...
        shader_paylod_assoc.pSubobjectToAssociate =
            &subobjects[current_subobj - 1];
        {
        ...
        // The root signature needs two subobjects: one to declare it, and one
        // to associate it with a set of symbols
        ...
        rt_local_root_sig.pLocalRootSignature = rt_root_signature.Get();
        {
            ...
            root_sig_assoc.pSubobjectToAssociate =
                &subobjects[current_subobj - 1];
        ...
        rt_global_root_sig.pGlobalRootSignature =
            rt_root_signature_dummy_global.Get();
        {
            // Declare the root signature
            D3D12_STATE_SUBOBJECT root_sig_obj = {0};
            root_sig_obj.Type =
        ...
        CHECK_ERR(device->CreateStateObject(&pipeline_desc,
                                            IID_PPV_ARGS(&rt_state_object)));
        ...
        // ===============================
        {
            ...
            CHECK_ERR(device->CreateCommittedResource(
                ...
                IID_PPV_ARGS(&rt_output_img)));
        }

        // ===============================
        // Create the shader resource heap
        // the resource heap has the pointers/views things to our output image
        // buffer and the top level acceleration structure
        // ===============================
        {
            ...
            CHECK_ERR(device->CreateDescriptorHeap(
                &heap_desc, IID_PPV_ARGS(&rt_shader_res_heap)));
            ...
            device->CreateUnorderedAccessView(rt_output_img.Get(), nullptr,
                                              &uav_desc, heap_handle);

            // Write the TLAS after the output image in the heap
            heap_handle.ptr += device->GetDescriptorHandleIncrementSize(
                D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

            D3D12_SHADER_RESOURCE_VIEW_DESC tlas_desc = {0};
            tlas_desc.Format = DXGI_FORMAT_UNKNOWN;
            tlas_desc.ViewDimension =
                D3D12_SRV_DIMENSION_RAYTRACING_ACCELERATION_STRUCTURE;
            tlas_desc.Shader4ComponentMapping =
                D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
            tlas_desc.RaytracingAccelerationStructure.Location =
                top_level_as->GetGPUVirtualAddress();
        ...
        // This is a table of pointers to the shader code and their respective
        // descriptor heaps
        // ===============================
        ...
        // An SBT entry is the program ID along with a set of params for the
        // program. the params are either 8 byte pointers (or the example
        // mentions 4 byte constants, how to set or use those?) Furthermore, the
        // stride between elements is specified per-group (ray gen, hit, miss,
        // etc) so it must be padded to the largest size required by any
        // individual entry. Note the order also matters for and should match
        // the instance contribution to hit group index. In this example it's
        // simple: our ray gen program takes a single ptr arg to the
        // rt_shader_res_heap, and our others don't take arguments at all
        {
            // 3 shaders and one that takes a single pointer param (ray-gen).
            // However, each shader binding table in the dispatch rays must have
            // its address start at a 64byte alignment, and use a 32byte stride.
            // So pad these out to meet those requirements by making each entry
            // 64 bytes
            ...
            CHECK_ERR(device->CreateCommittedResource(
                &props, D3D12_HEAP_FLAG_NONE, &res_desc,
                D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
                IID_PPV_ARGS(&rt_shader_table)));

            // Map the SBT and write our shader data and param info to it
            uint8_t *sbt_map = nullptr;
            CHECK_ERR(rt_shader_table->Map(
                0, nullptr, reinterpret_cast<void **>(&sbt_map)));

            // First we write the ray-gen shader identifier, followed by the ptr
            // to its descriptor heap
            std::memcpy(sbt_map,
                        rt_pipeline_props->GetShaderIdentifier(L"RayGen"),
                        D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
            // Set descriptor table binding for the raygen program
            //*reinterpret_cast<D3D12_GPU_DESCRIPTOR_HANDLE*>(sbt_map) =
            ...
            // Each entry must start at an alignment of 32bytes, so offset by
            // the required alignment
            sbt_map += 2 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT;

            std::memcpy(sbt_map,
                        rt_pipeline_props->GetShaderIdentifier(L"Miss"),
                        D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
            sbt_map += 2 * D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;

            std::memcpy(sbt_map,
                        rt_pipeline_props->GetShaderIdentifier(L"HitGroup"),
                        D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
...

    {
        ...
        // We know the gpu-side data copy is done now, so release the upload
        // buffer and the AS scratch buffers
        upload = nullptr;
    ...
    while (!done) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_WINDOWEVENT &&
                ...
                D3D12_CPU_DESCRIPTOR_HANDLE rtv_handle =
                    rtv_heap->GetCPUDescriptorHandleForHeapStart();
                for (int i = 0; i < 2; ++i) {
                    CHECK_ERR(swap_chain->GetBuffer(
                        i, IID_PPV_ARGS(&render_targets[i])));
                ...
                // Resize the output buffer
                {
                    ...
                    device->CreateUnorderedAccessView(
                        rt_output_img.Get(), nullptr, &uav_desc, heap_handle);
                }
        ...
        // Back buffer will be used as render target
        {
            ...
            res_barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
            res_barrier.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
            res_barrier.Transition.pResource =
                render_targets[back_buffer_idx].Get();
        ...
        cmd_list->ClearRenderTargetView(render_target, clear_color.data(), 0,
                                        nullptr);

        // Ray trace it!
        // Bind our descriptor heap with the output texture and the accel.
        // structure
        std::vector<ID3D12DescriptorHeap *> heaps = {rt_shader_res_heap.Get()};
        cmd_list->SetDescriptorHeaps(heaps.size(), heaps.data());

        // Transition the output buffer back to a unordered access, since we'll
        // have it copy source from blitting the image
        {
            D3D12_RESOURCE_BARRIER res_barrier;
            res_barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
            res_barrier.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
        ...
        dispatch_rays.RayGenerationShaderRecord.StartAddress =
            rt_shader_table->GetGPUVirtualAddress();
        // The shader identifier + its descriptor table
        dispatch_rays.RayGenerationShaderRecord.SizeInBytes =
            D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES + sizeof(uint64_t);

        // Miss is next, followed by hit, each is just a shader identifier
        dispatch_rays.MissShaderTable.StartAddress =
            rt_shader_table->GetGPUVirtualAddress() +
            2 * D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
        dispatch_rays.MissShaderTable.SizeInBytes =
            D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
        dispatch_rays.MissShaderTable.StrideInBytes =
            D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;

        dispatch_rays.HitGroupTable.StartAddress =
            rt_shader_table->GetGPUVirtualAddress() +
            4 * D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
        dispatch_rays.HitGroupTable.SizeInBytes =
            D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
        dispatch_rays.HitGroupTable.StrideInBytes =
            D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
        ...
        // Transition the raytrace image to copy source and the back buffer to
        // copy dest
        {
        ...
        cmd_list->CopyResource(render_targets[back_buffer_idx].Get(),
                               rt_output_img.Get());

        // If we're taking a screenshot also copy the rendered image to our
        // readback buffer
        if (take_screenshot) {
            // The output image is already in the copy source state so we don't
            // need a barrier here
            ...
            dst_desc.PlacedFootprint.Footprint.Format =
        ...
        // TODO: Seems like sometimes this fence doesn't really wait how I
        // expect it to and I get flickering?
        if (fence->GetCompletedValue() < signal_val) {
            CHECK_ERR(fence->SetEventOnCompletion(signal_val, fence_evt));
            WaitForSingleObject(fence_evt, INFINITE);
        }
    ...
    for (UINT i = 0; i < desc->NumSubobjects; i++) {
        ...
        switch (desc->pSubobjects[i].Type) {
            case D3D12_STATE_SUBOBJECT_TYPE_GLOBAL_ROOT_SIGNATURE:
                wstr << L"Global Root Signature 0x"
                     << desc->pSubobjects[i].pDesc << L"\n";
                break;
            ...
            case D3D12_STATE_SUBOBJECT_TYPE_EXISTING_COLLECTION: {
                ...
                wstr << ExportTree(1, collection->NumExports,
                                   collection->pExports);
                break;
            }
            case D3D12_STATE_SUBOBJECT_TYPE_SUBOBJECT_TO_EXPORTS_ASSOCIATION: {
                ...
                auto association =
                    static_cast<const D3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION *>(
                        desc->pSubobjects[i].pDesc);
                UINT index = static_cast<UINT>(
                    association->pSubobjectToAssociate - desc->pSubobjects);
                wstr << index << L"])\n";
                for (UINT j = 0; j < association->NumExports; j++) {
                    wstr << L"|  [" << j << L"]: " << association->pExports[j]
                         << L"\n";
                }
                break;
            }
            case D3D12_STATE_SUBOBJECT_TYPE_DXIL_SUBOBJECT_TO_EXPORTS_ASSOCIATION: {
                ...
                auto association = static_cast<
                    const D3D12_DXIL_SUBOBJECT_TO_EXPORTS_ASSOCIATION *>(
                    desc->pSubobjects[i].pDesc);
            ...
            case D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_SHADER_CONFIG: {
                ...
                auto config =
                    static_cast<const D3D12_RAYTRACING_SHADER_CONFIG *>(
                        desc->pSubobjects[i].pDesc);
                wstr << L"|  [0]: Max Payload Size: "
                     << config->MaxPayloadSizeInBytes << L" bytes\n";
